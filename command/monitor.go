...
func CmdMonitor(c *cli.Context) {
	...
	if err != nil {
		log.Print(err)
		os.Exit(ng_agent.MONITOR_UNKNOWN)
	}
...
func getMonitorJSON(plugin_name string, plugin_option string) ([]byte, error) {
	monitor_request := ng_agent.MonitorRequest{
		Api_Key:       "",
...
func parseMonitorJSON(jsonStr string) (ng_agent.MonitorResponse, error) {
	var m ng_agent.MonitorResponse
